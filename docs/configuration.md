...
| `BIRDCLAW_FXTWITTER_ENABLED`                     | Set to `1` to use free FxTwitter first; when paid 6551 is also enabled it remains a thresholded REST-only reserve                                                                      |
...
| `BIRDCLAW_6551_ENABLED`                          | Set to `1` to enable paid 6551; combined Fx mode keeps it REST-only behind the failure, cooldown, and budget gates                                                                     |
...
| `BIRDCLAW_6551_PAID_FALLBACK_FAILURE_THRESHOLD`  | Consecutive total FxTwitter failures required before paid REST reserve; defaults to `3`                                                                                                |
| `BIRDCLAW_6551_PAID_FALLBACK_COOLDOWN_MINUTES`   | Minimum time between claimed paid REST reserve attempts; defaults to `360`                                                                                                             |
| `BIRDCLAW_6551_PAID_DAILY_REQUEST_BUDGET`        | Hard UTC-day limit for actual paid HTTP attempts, including retries; defaults to `24`, `0` disables paid requests                                                                      |
...
| `BIRDCLAW_LOCAL_COLLECTOR_STARTUP_MAX_RESULTS`   | Deeper Following result limit used until the first successful Home sync after process start; defaults to `600`, then regular polling returns to `BIRDCLAW_LOCAL_COLLECTOR_MAX_RESULTS` |
...
Paid-guard variables use their conservative defaults only when omitted. An
explicit invalid, negative, fractional, or zero daily budget disables paid
requests; invalid threshold or cooldown values hold paid recovery closed.
