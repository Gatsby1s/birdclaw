...
Schema version 22 is additive. `bin/migration-backup.mjs` requires a verified pre-v22 SQLite backup before opening the writable production database. Confirm enough volume capacity and an independent, integrity-checked backup before deployment. Prefer a Railway volume snapshot when the plan supports it; otherwise copy the SQLite online-backup snapshot off the volume, restrict access, and verify its SHA-256 plus SQLite integrity before migration. The in-volume pre-migration file alone is not an independent disaster-recovery copy. For subsequent migrations, also back up the original documents and media under `person-archive`, preserving their storage keys; a database-only copy does not preserve file evidence. Rollback uses a forward-moving code fix/release; do not open schema 22 with a binary that only understands schema 21, and do not remove captured records to downgrade.
